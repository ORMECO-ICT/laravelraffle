...
class RaffleEntriesDataTable extends DataTable
{
    /**
     ...
     */
    public function query(ConsumerData $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
                    ->setTableId('table_raffle_entries')
                    ->columns($this->getColumns())
                    // ->minifiedAjax()
                    ->minifiedAjax( route('registration.ajax-raffle-entries') )
                    ->searchDelay(1000)
                    //->dom('Bfrtip')
                    ->orderBy(1, 'asc')
                    ->selectStyleSingle()
    ...
    {
        return 'Raffle_entries_' . date('YmdHis');
    }
}
